...
import com.cv.inv.entry.common.VouStatusCompleterTableModel;
...
public class VouStatusAutoCompleter implements KeyListener, SelectionObserver {
    ...
    private VouStatusCompleterTableModel vouTableModel;
    ...
    public VouStatusAutoCompleter(JTextComponent comp, List<VouStatus> list,
        ...
        vouTableModel = new VouStatusCompleterTableModel(list);
        table.setModel(vouTableModel);
        table.setSize(50, 50);
    ...
    public void mouseSelect() {
        if (table.getSelectedRow() != -1) {
            vouStatus = vouTableModel.getVouStatus(table.convertRowIndexToModel(
                    table.getSelectedRow()));
            ((JTextField) textComp).setText(vouStatus.getStatusDesp());
            if (editor == null) {
                if (selectionObserver != null) {
                    selectionObserver.selected("Location", vouStatus.getStatusDesp());
                }
            }
        }
        popupOpen = false;
