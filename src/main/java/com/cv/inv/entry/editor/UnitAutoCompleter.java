...
public class UnitAutoCompleter implements KeyListener {
    ...
    public void mouseSelect() {
        if (table.getSelectedRow() != -1) {
            stock = unitTableModel.getStockUnit(table.convertRowIndexToModel(
                    table.getSelectedRow()));
            ((JTextField) textComp).setText(stock.getItemUnitName());
            if (editor == null) {
                if (selectionObserver != null) {
                    selectionObserver.selected("Stock", stock.getItemUnitCode());
                }
            }
        }
