...
import com.cv.inv.entry.common.StockTypeTableModel;
...
public class StockTypeAutoCompleter implements KeyListener {
    ...
    public void mouseSelect() {
        if (table.getSelectedRow() != -1) {
            type = stockTypeTableModel.getType(table.convertRowIndexToModel(
                    table.getSelectedRow()));
            ((JTextField) textComp).setText(type.getItemTypeName());
            if (editor == null) {
                if (selectionObserver != null) {
                    selectionObserver.selected("Description", type.getItemTypeName());
                }
            }
        }
    ...
    public void setStockType(StockType type) {
        this.type = type;
        textComp.setText(type.getItemTypeName());
