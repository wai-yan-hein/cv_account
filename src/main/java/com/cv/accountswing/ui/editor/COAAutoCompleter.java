 ...
 * @author Lenovo
 */
public class COAAutoCompleter implements KeyListener {

    private static final org.slf4j.Logger log = LoggerFactory.getLogger(COAAutoCompleter.class);
    ...
    public COAAutoCompleter(JTextComponent comp, List<ChartOfAccount> list,
        ...
        table.getColumnModel().getColumn(2).setPreferredWidth(50);//Name
        ...
        if (table.getSelectedRow() != -1) {
            ...
            if (editor == null) {
                if (selectionObserver != null) {
                    selectionObserver.selected("COA", coa.getCode());
                }
            }
        }
    ...
    private Action acceptAction = new AbstractAction() {
        ...
        /*Component tf = (JComponent) e.getSource();
                    COAAutoCompleter completer = (COAAutoCompleter) tf.getClientProperty(AUTOCOMPLETER);
                    
                    if (completer.table.getSelectedRow() != -1) {
                    coa = cOATableModel.getCOA(completer.table.convertRowIndexToModel(
                    completer.table.getSelectedRow()));
                    ((JTextField) completer.textComp).setText(coa.getCoaNameEng());
                    }
