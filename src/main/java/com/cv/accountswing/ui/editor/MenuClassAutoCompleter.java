...
import com.cv.accountswing.ui.cash.common.MenuClassTableModel;
 ...
 * @author Lenovo
 */
public class MenuClassAutoCompleter implements KeyListener {

    private static final org.slf4j.Logger log = LoggerFactory.getLogger(MenuClassAutoCompleter.class);
    private JTable table = new JTable();
    private JPopupMenu popup = new JPopupMenu();
    private JTextComponent textComp;
    private static final String AUTOCOMPLETER = "AUTOCOMPLETER"; //NOI18N
    private MenuClassTableModel menuModel;
    private VMenuClass desp;
    ...
    public MenuClassAutoCompleter() {
    }

    public MenuClassAutoCompleter(JTextComponent comp, List<VMenuClass> list,
            AbstractCellEditor editor, VDescriptionService descriptionService) {
        this.textComp = comp;
        this.editor = editor;
        this.descriptionService = descriptionService;
        if (Global.listMenuClass.isEmpty()) {
            Global.listMenuClass = this.descriptionService.getMenuClass();
        }
        
        textComp.putClientProperty(AUTOCOMPLETER, this);
        textComp.setFont(Global.textFont);
        menuModel = new MenuClassTableModel(Global.listMenuClass);
        table.setModel(menuModel);
    ...
    public void mouseSelect() {
        if (table.getSelectedRow() != -1) {
            desp = menuModel.getRemark(table.convertRowIndexToModel(
                    table.getSelectedRow()));
            ((JTextField) textComp).setText(desp.getMenuClass());
            if (editor == null) {
                if (selectionObserver != null) {
                    selectionObserver.selected("Description", desp.getMenuClass());
                }
            }
        }
    ...
    Action showAction = new AbstractAction() {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            MenuClassAutoCompleter completer = (MenuClassAutoCompleter) tf.getClientProperty(AUTOCOMPLETER);
            if (tf.isEnabled()) {
    ...
    public VMenuClass getAutoText() {
        return desp;

    }

    public void setAutoText(VMenuClass desp) {
        this.desp = desp;
        textComp.setText(desp.getMenuClass());
